...
class Template {
    ...
    private function compileTPL($template_dir, $tpl_name = false) {
        ...
        */
        ...
        $compiling = preg_replace_callback('/{(.*?)\.(.*?)}/', array( &$this, 'loadExtraVariables'), $compiling);
    ...
    private function loadExtraVariables($matches) {
        ...
        switch ($matches[1]) {
            case 'user':
                ...
                break;
            case 'config':
                return '<?php echo $this->config->get(\'' . $test[0]. '\');?>';
                break;
            case 'lang':
                return '<?php echo $this->language->get(\'' . $test[0]. '\');?>';
                break;
            default:
                break;
        }
        ...
        $return = $this->getPhpVariable($test, $value, $start);
        
        return '<?php echo ' . $return. ';?>';
    }

    private function loadVariable($matches) {
        $test = explode('.', $matches[1]);
        if(!array_key_exists($test[0], $this->variables))
            throw new \Exception();


        $return = $this->getPhpVariable($test, $this->variables[$test[0]]);
